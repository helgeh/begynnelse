...
try {
  ...
  if (err.errno === -17) log('DB file already exists.')
  else log('Something wrong opening DB file', err)
} finally {
...
const updateUserDetailsStmt = db.prepare(
  'UPDATE users SET details = ? WHERE id = ?',
)
...
const userByEmailStmt = db.prepare(
  'SELECT id, name, email, password, details FROM users WHERE email = ?',
)

async function addUser(email, password, details) {
  const user = getUserByEmail(email)
  const hash = await hashPassword(password)
  if (user && user.email === email) throw new Error('User already exists')
...
function removeUser(id) {
  ...
  if (user) removeUserByIdStmt.run(id)
...
const insertLinksStmt = db.prepare(
  'INSERT INTO links (name, url, user) VALUES (?, ?, ?)',
)
const insertCompleteLinksStmt = db.prepare(
  'INSERT INTO links (name, url, category, tags, icon, user) VALUES (:name, :url, :category, :tags, :icon, ?)',
)
const updateLinkStmt = db.prepare(
  'UPDATE links SET name = :name, url = :url, icon = :icon, category = :category, tags = :tags WHERE id = :id',
)
const updateLinkIconStmt = db.prepare(
  'UPDATE links SET icon = :icon WHERE id = :id',
)
const updateLinkCategoryStmt = db.prepare(
  'UPDATE links SET category = :category WHERE id = :id',
)
const updateLinkTagsStmt = db.prepare(
  'UPDATE links SET tags = :tags WHERE id = :id',
)
...
const linksByUserStmt = db.prepare(
  'SELECT links.id, links.name, url, icon, categories.name AS categoryName, categories.title AS categoryTitle, tags FROM links LEFT JOIN categories ON categories.id = links.category WHERE links.user = ? ORDER BY links.id',
)
const linkByIdStmt = db.prepare(
  'SELECT id, name, url, icon, category, tags, user FROM links WHERE id = ?',
)
