<template>
    ...
    >
        <v-sheet class="bg-primary pa-2" min-height="500px" rounded>
            <v-icon @click="videoVisible = false" icon="mdi-close" class="position-absolute top-0 right-0 me-4 mt-4 pa-4 bg-black" style="z-index: 999" />
...
<script setup>
    ...
    function downloadVideo(data) {
        ...
        anchor.value.setAttribute('download', item.value.substr(item.value.lastIndexOf('/') + 1))
        anchor.value.click()
    }
    ...
    function fetchVideos() {
        ...
        .then(files => {
            if (isAdmin)
                files.forEach(item => (item.alt = true) && (item.altIcon = 'mdi-download'))
            items.value = files
