  ...
  install(app, options) {
    const actionHandlers = [
      ...
      [
        'seekbackward',
        (details) => {
          if (callbacks['seekbackward']) callbacks['seekbackward'](details)
        },
      ],
      [
        'seekforward',
        (details) => {
          if (callbacks['seekforward']) callbacks['seekforward'](details)
        },
      ],
      ['seekto', null], //(details) => { /* ... */ }],
    ...
    app.provide('mediasession', {
      setCallback: function (key, cb) {
        if (Object.keys(callbacks).indexOf(key) >= 0) callbacks[key] = cb
      },
